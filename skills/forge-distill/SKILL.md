...
**One format, two producers, one consumer.** The distill format is **single-sourced in `templates/distill.md`**. This skill is the in-system producer of that format; a web AI pasting the same file is the other producer; `forge-ingest` is the consumer for both. Do not restate the format here — read the template and apply the in-system adaptations below. **Named coupling:** any format change must land in `templates/distill.md` and be honored here (and vice versa).
...
### Step 2 — Compose (per `templates/distill.md`)
Apply `templates/distill.md` in full: user-perspective priority, faithfulness (no invented detail), neutral register on the user, epistemic tagging (`decided` vs. `discussed` vs. `(AI-suggested, unvalidated)`), ideation attribution, the **Thought Direction (Steering Trace)** section with a blunt steering read, and the never-omitted **Risks & Unexamined Assumptions**. Omit sections that would be empty or forced.

**In-system adaptations** (the only deltas from `templates/distill.md`):
...
## Related
- `templates/distill.md` — the format (web producer and this skill share it).
- `skills/forge-ingest/SKILL.md` — consumer.
- `templates/schema.md` — producer boundary.
