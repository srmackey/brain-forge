...
- Note inline hints (`#todo`, `#revisit`, `suggested-title:`, etc.) and any `## Action Items / Next Steps` section. Engagement language in the capture (grabbed / extended unfinished ideas) is still-live, even without those tags.
...
**These five patterns describe an AI summarizing a session with the user.** Material that is not that — a saved article, a spec someone else wrote, a document pasted in for reference — cannot fail any of them, and `clean` would assert a screen that never really ran. Give it `capture-quality: not-applicable (third-party material)`. Judge from provenance: a `source:` naming a session or an assistant means screen it, and prose written to an audience that is not the user, with no record of the user in it, generally means this instead. When genuinely unsure, screen it; a false flag costs a sentence and a false pass costs the wiki.

**The risk with third-party material is different, and it is real.** Nothing in a saved article stops a wiki page written from it reading, two years later, as though the thinking was the user's. So a `not-applicable` verdict carries one requirement into Step 6: the wiki contribution says whose material it is, and summarizes rather than reproducing at length. That is the whole screen for this class, and it is not optional.

Output a per-capture verdict — `capture-quality: clean`, `capture-quality: flagged (patterns N, …)`, or `capture-quality: not-applicable (third-party material)` — that **travels with the capture** through routing and synthesis and **must appear** in the human digest and the Step 7 `raw/_log.md` entry. The screen never blocks ingest by itself; default handling is flag + neutralize (Step 6). **Severe case** (untagged AI-suggested conclusions dominate the capture, so a faithful wiki write would require guesswork): propose **`hold`** and a cleaned sibling capture in `raw/` (same date, marked as a replacement). Do not ingest until the user decides.
...
- **Still live.** A capture can be knowledge and also something to come back to. Wiki-bound captures only (held stays in `raw/` and the digest). After the wiki write, collect still-live items from Action Items, `#todo`, `#revisit`, and engagement-tagged unfinished ideas in this capture. Do not wait for the owner to mark up the drop. Append each as one dated line under `## Still live` on [[wiki/projects]] (create that section if it is missing; create the page only if no projects hub exists). Each line is the item, the real date, and `Source: [[raw/YYYYMM/YYYYMMDD-filename.md]]`. Skip a line that is already on the list (same source + same idea). Do not promote ungrabbed AI suggestions. Do not write still-live items as if they were decided knowledge. Do not create a `focus/` tree. Dismiss is the owner's: they strike or move a line; this skill does not auto-decay. Include the lines added (or "none") in the human digest and the `raw/_log.md` entry.
...
- **Third-party material (`not-applicable` verdict):** attribute it in the wiki. Say it is someone else's, summarize rather than reproducing at length, and never let it settle into a page as though the user had thought it. Provenance to the capture is not enough on its own, since a `Source:` line says where the file came from and not whose idea it was.
