...
- **Idea finder** — captures with `type: idea`. Home of the convention: `_brain-forge/schema.md` Idea captures. Default finder `wiki/ideas.md` if present (create it when ingesting the first idea if the vault has none). Row only: date, gist, about, pointer to raw, produced if any. Wire the domain hub with a pointer. Do not dump the capture onto the finder. Do not create a wiki page per idea.
