...
2. **Create the starting layout**, only for what is absent. These are the owner's from birth and are never touched again. This comes before seeding, because the seeded primers land in a directory this step makes.
   ...
   - `primers/_index.md`, the slim human-readable primer catalog. Same frontmatter, plus a line for each catalog-eligible primer this install ships, pointing at the `primers/` path step 3 seeds it to. **Read eligibility off the framework folder step 1 just copied:** every file in `_brain-forge/primers/` whose own frontmatter carries `signal: true`. Do not work from a list of filenames written here. The shipped primer set is allowed to change, and a list in this step would drift from it exactly as step 5's owner-state list did. Reading the folder is also why this step can describe files step 3 has not seeded yet: their source is already on disk.
